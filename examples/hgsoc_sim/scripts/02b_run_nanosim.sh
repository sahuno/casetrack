#!/bin/bash
# 02b_run_nanosim.sh — NanoSim (cDNA) + minimap2 splice pipeline (optional RNA lane).
#
# For each specimen with an `rna:` block (produced by 01b_prepare_expression.py
# under cohort/<PATIENT>/<SPECIMEN>/rna/) this script:
#   1. Runs NanoSim transcriptome-mode with the per-specimen expression.tsv
...
#   4. Sorts and indexes with samtools → rna/sim.srt.bam.
#
# DNA assays are NOT processed here — they go through scripts/02_run_visor.sh.
#
# Requires:
#   - scripts/00b_fetch_gencode.sh (transcripts.fa)
#   - scripts/00c_fetch_nanosim_model.sh (pre-trained model)
#   - scripts/01b_prepare_expression.py (per-specimen rna/expression.tsv)
#
...
# all files sharing a 'training' prefix; fall back to the longest common prefix
# of .pkl files if the model ships with a different convention.
...
# ── Iterate over specimens that opted into the RNA lane ──────────────────────
...
for patient_dir in "${patients[@]}"; do
    ...
    for spec_dir in "$patient_dir"/*/; do
        ...
        if [[ -s "$out_bam" ]]; then
            echo "[02b] $patient/$specimen/$RNA_LEAF — already simulated, skipping"
            continue
        fi

        target_n=$(awk -F'\t' 'NR > 1 {s += $2} END {print s}' "$expression_tsv")
        echo "[02b] $patient/$specimen/$RNA_LEAF — target ${target_n} reads"
        ...
        echo "[02b] $patient/$specimen/$RNA_LEAF — NanoSim"
        ...
        if [[ ! -s "$reads_fq" ]]; then
            echo "Error: NanoSim produced no reads for $patient/$specimen/$RNA_LEAF" >&2
            exit 1
        fi
        ...
        echo "[02b] $patient/$specimen/$RNA_LEAF — minimap2 splice + samtools sort"
        ...
        echo "[02b] $patient/$specimen/$RNA_LEAF → $out_bam"
    done
done

if [[ $any -eq 0 ]]; then
    echo "[02b] no specimens with an rna/ directory — nothing to simulate."
    exit 0
fi

echo "[02b] all opt-in RNA specimens simulated."
