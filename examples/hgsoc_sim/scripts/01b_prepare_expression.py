#!/usr/bin/env python3
"""01b_prepare_expression.py — emit per-RNA-assay NanoSim expression TSVs.

For every (patient, specimen, ONT-RNA assay) in config.yaml, generates:

    sandbox/hgsoc_sim/cohort/<PATIENT>/<SPECIMEN>/<ASSAY>/expression.tsv
...
Model:
  ...
  2. Per-assay `expression.gene_multipliers` in config.yaml scales baseline
     TPMs by the named gene (e.g. `BRCA1: 0.1` drops BRCA1 transcripts 10×
     in a tumor with BRCA1 LOF).
  3. Scaled TPMs are normalized to sum to 1e6 (proper TPM), then est_counts
     are assigned proportionally to sum to the assay's `n_reads` target.
...
RNA_ASSAY_TYPES: tuple[str, ...] = ("ONT-RNA",)
...
# Fixed seed string for the baseline. Change to regenerate baseline for
# all specimens. Keeping stable across phase f / g bumps.
BASELINE_SEED_STRING = "hgsoc_sim_baseline_v1"


def _seed_from(s: str) -> int:
    """Stable 32-bit int seed derived from an arbitrary string."""
    return int(hashlib.md5(s.encode()).hexdigest()[:8], 16)


def build_baseline_tpm(transcripts: pd.DataFrame) -> np.ndarray:
    """Deterministic log-normal baseline TPM, one value per transcript."""
...
def apply_multipliers(
    ...
    """Return (scaled TPMs, list of warnings) after applying gene-level overrides."""
...
def assign_counts(scaled_tpm: np.ndarray, n_reads: int) -> np.ndarray:
    """Normalize scaled_tpm to sum to 1e6, then assign integer est_counts
    summing exactly to n_reads."""
    if scaled_tpm.sum() <= 0:
        # All transcripts zeroed out (e.g. a 0× multiplier on a cohort with
        # only one gene). Distribute reads uniformly as a fallback so
        # NanoSim doesn't explode.
        return np.full(len(scaled_tpm), n_reads // len(scaled_tpm), dtype=int)

    tpm = scaled_tpm * 1e6 / scaled_tpm.sum()
    # proportional integer allocation with a correction for the rounding drift
    raw = tpm * n_reads / 1e6
    counts = np.floor(raw).astype(int)
    delta = n_reads - counts.sum()
    if delta > 0:
        # Distribute the remainder to the transcripts with the largest
        # fractional parts.
        frac = raw - counts
        idx = np.argsort(frac)[::-1][: int(delta)]
        counts[idx] += 1
    elif delta < 0:
        # Over-allocated (shouldn't happen with floor, but guard anyway).
...
def _emit_for_assay(
    ...
    n_reads = int(assay_cfg.get("n_reads", 10000))
    multipliers = (assay_cfg.get("expression") or {}).get("gene_multipliers") or {}
    scaled, warns = apply_multipliers(transcripts, baseline_tpm, multipliers)
    counts = assign_counts(scaled, n_reads)
    # Normalized TPMs for the informational `tpm` column.
    if scaled.sum() > 0:
    ...
    for patient in cfg["cohort"]:
        ...
        for spec in patient["specimens"]:
            ...
            for assay in spec.get("assays") or []:
                atype = assay["type"]
                if atype not in RNA_ASSAY_TYPES:
                    continue
                out_dir = cohort_dir / pid / suffix / atype
                total_reads, warns = _emit_for_assay(
                    out_dir, transcripts, baseline_tpm, assay
                )
                mults = (assay.get("expression") or {}).get("gene_multipliers") or {}
                mult_desc = ", ".join(
                    f"{g}×{m}" for g, m in mults.items()
                ) or "baseline only"
                summary.append(
                    f"  {pid}/{suffix}/{atype}: {total_reads} reads, "
                    f"{mult_desc} → {out_dir}/expression.tsv"
                )
                all_warns.extend(
                    f"  {pid}/{suffix}/{atype}: {w}" for w in warns
                )

    if not summary:
        print("[01b] no ONT-RNA assays in config.yaml — nothing to do.")
        return 0

    print(f"[01b] emitted expression TSVs for {len(summary)} RNA assay(s):")
    for line in summary:
        print(line)
    if all_warns:
